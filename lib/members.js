...
// Parents can only lock in a day about a week out — not 3 weeks ahead.
...
// pick them one day at a time, only about a week ahead. The other limits are:
// don't spend more than 4, don't book the past, don't run past the expiry
// date, and one lesson per day.
export function bookWindowBlocked(date) {
  const n = daysAhead(date);
  if (n < 1) return "Please pick a day from tomorrow onward.";
  if (n > BOOK_AHEAD_DAYS) {
    return "I only open the schedule about a week ahead. Please pick an earlier day.";
  }
  return null;
}
