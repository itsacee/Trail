import { fromPayment, membershipSummary, MEMBER_PERIOD_DAYS } from "./members.js";
import { loadLessons, scheduledFor } from "./lessons.js";
...
// A membership stops occupying one of the 15 roster spots as soon as all four
// lesson credits are booked. Keep the Stripe payment for history/accounting;
// only remove the used-up membership from the active roster and capacity count.
export function membershipsWithCredits(memberships, storedLessons) {
  return (memberships || []).filter((membership) => {
    const scheduled = scheduledFor(membership, storedLessons || { lessons: [], voids: [] });
    return membershipSummary(membership, scheduled).remaining > 0;
  });
}
...
export async function getMembershipCapacity(stripeKey, requestedEmail = "") {
  const [paidMemberships, pendingCheckouts, storedLessons] = await Promise.all([
    listActiveMemberships(stripeKey),
    listPendingMembershipCheckouts(stripeKey),
    loadLessons(),
  ]);
  const activeMemberships = membershipsWithCredits(paidMemberships, storedLessons);
  return {
    activeMemberships,
    pendingCheckouts,
